...
const QueueList2 = ({
  queue = [], // Set default value as an empty array
  timeAgo,
  open,
  setOpen,
  spotify,
    ...
    <AnimatePresence mode="sync">
      {queue.length > 0 ? (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={"w-full overflow-hidden flex flex-col mt-1 p-2"}
        >
          <motion.ul
            layoutScroll
            className={
          ...
          >
            <div className="fixed top-0 z-[60] p-3 flex items-center justify-end w-full">
              <button
                className="m-2 rounded-xl py-1 px-2 bg-green-900 text-xl font-black text-center hover:opacity-[0.5] transition-all"
                style={{ backgroundColor: mutedColor, color: lightMutedColor }}
                onClick={addSong}
              >
                +
              </button>
              ...
              {queue.map((q: any, index: number) => (
                ...
                >
                  ...
                  <div
                    ...
                    className={
                      q?.current
                        ? "drop-shadow-xl px-2 flex flex-col justify-center w-full truncate cursor-pointer"
                        : "drop-shadow-xl px-2 flex flex-col justify-center w-full truncate"
                    }
                  >
                    ...
                    <AnimatePresence mode="sync">
                      {q.current && (
                        <motion.div
                          ...
                          animate={{ height: "50%", width: "100%", opacity: 1 }}
                        ...
                        >
                          ...
                          <div className="flex justify-between">
                            <p className="font-bold">
                              {prettyMilliseconds(spotify?.progress || 0, {
                                colonNotation: true,
                                secondsDecimalDigits: 0,
                              })}
                            </p>
                            <p className="font-bold">
                              {prettyMilliseconds(spotify?.duration || 0, {
                                colonNotation: true,
                                secondsDecimalDigits: 0,
                              })}
                            </p>
                      ...
                      {!q?.current && q?.playedAt > 1 && (
                        <motion.div>
                          <p className="w-full text-sm font-regular text-left">
                            {q?.current ? "Playing since" : "Played"}{" "}
                            {timeAgo?.format(q?.playedAt)}
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
        ...
        <motion.div
          ...
          exit={{ opacity: 0 }}
          id="current"
          className="w-full h-screen flex flex-col text-center items-center justify-center overflow-hidden"
        >
